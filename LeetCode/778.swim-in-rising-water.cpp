...
public:
    int swimInWater(vector<vector<int>>& grid) {
        ...
        return bfs(0, 0, grid, grid[0][0], pq);
    }
private:
    int bfs(int x, int y, vector<vector<int>>& grid, int ev, priority_queue<pair<int, pair<int, int>>>& pq) {
        int rlen = grid.size();
        int clen = grid[0].size();
        if (x == clen-1 && y == rlen-1) return ev;
        int t[4][2] = {
            {-1, 0},
            {0, -1},
            {1, 0},
        ...
        for (int i = 0; i < 4; i++) {
            int tx = x + t[i][0]; 
            int ty = y + t[i][1];
            if (tx >= 0 && tx < clen && ty >= 0 && ty < rlen) {
                if (grid[tx][ty] != -1) {
                    pq.push(make_pair(-grid[tx][ty], make_pair(tx, ty)));
                }
            }
        }
        auto pa = pq.top();
        pq.pop();
        if (-pa.first > ev) ev = -pa.first;
        int mx = pa.second.first;
        int my = pa.second.second;
        grid[mx][my] = -1;
        return bfs(mx, my, grid, ev, pq);
    }
};
