...
class Solution {
    public int numDecodings(String s) {
        ...
        for (int i = n-1; i >= 0; i--) {
            if (chs[i] == '0') {
                ...
                dp[i] = dp[i+1];
                if (i < n-1 && (chs[i] == '1' || (chs[i] == '2' && chs[i+1] < '7'))) {
                    dp[i] += dp[i+2];
                }
            }
        }
    ...
    public int nd(String s) {
        int n = s.length();
        Integer[] mem = new Integer[n];
        return n == 0 ? 0 : nd(0, s.toCharArray(), mem);
    }

    public int nd(int p, char[] chs, Integer[] mem) {
        int n = chs.length;
        if (p == n) {
            return 1;
        }
        ...
        int res = nd(p+1, chs, mem);
        if (p+1 < n && (chs[p] == '1' || (chs[p] == '2' && chs[p+1] < '7'))) {
            res += nd(p+2, chs, mem);
        }
        mem[p] = res;
        return res;
